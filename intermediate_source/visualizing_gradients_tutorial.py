...
학습 중 발생하는 `변화도 소실 또는 폭발 <https://arxiv.org/abs/1211.5063>`__과 같은 
문제를 디버깅할 수 있습니다.
...
# 
# 변화도 시각화의 중요성을 설명하기 위해, 배치 정규화를 적용한
...
######################################################################
# 이제 더미 데이터(dummy data)를 준비하여 두 버전의 모델을 생성하고
# 옵티마이저를 초기화합니다.
#
...
######################################################################
# 내부 계층 중 하나를 조사하여 배치 정규화가 하나의 모델에만 
# 적용되는지 확인할 수 있습니다.
#
...
######################################################################
# 이제 계산한 평균 변화도를 그래프로 나타내어 네트워크 깊이에 따라 
# 값이 어떻게 변하는지 확인할 수 있습니다.
# 배치 정규화를 적용하지 않으면 중간 계층의 변화도가
